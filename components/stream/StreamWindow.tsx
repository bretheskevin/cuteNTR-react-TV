...
import {Focusable, isTV, OVERSCAN} from '../tv';
...
const JpegVideoView =
  requireNativeComponent<JpegVideoViewProps>('JpegVideoView');
...
class SingletonJpegVideoView extends Component<JpegVideoViewProps> {
  ...
  constructor(props: JpegVideoViewProps) {
    ...
    if (SingletonJpegVideoView.instance) {
      console.warn(
        'SingletonJpegVideoView already exists. Reusing the existing instance.',
      );
    } else {
...
class StreamWindow extends Component<StreamWindowProps, StreamWindowState> {
  ...
  constructor(props: StreamWindowProps) {
    ...
    this.frameReadyListener = EventRegister.addEventListener(
      'frameReady',
      this.renderFrame,
    );
    ...
    if (this.props.bothViewEnabled) {
      if (isTop) {
        ...
        this.setState(prev => ({
          ...
          framesBottom: prev.recording
            ? [...prev.framesBottom, uri]
            : prev.framesBottom,
        }));
  ...
  startFPSCounter = () => {
    ...
    this.fpsInterval = setInterval(() => {
      if (this.props.bothViewEnabled) {
        this.setState({
          fpsTop: this.frameCountTop,
          fpsBottom: this.frameCountBottom,
        });
        this.frameCountTop = 0;
        this.frameCountBottom = 0;
      } else {
  ...
  async saveRecording() {
    const recordingDurationInSeconds =
    ...
    if (this.props.bothViewEnabled) {
      ...
      if (framesTop.length === 0 || framesBottom.length === 0) {
        Alert.alert(
          'No frames captured',
          'Recording did not capture frames from both streams.',
        );
      ...
      const topFPS =
        Math.round(framesTop.length / recordingDurationInSeconds) || 30;
      const bottomFPS =
        Math.round(framesBottom.length / recordingDurationInSeconds) || 30;

      const maxFrameCount = Math.max(framesTop.length, framesBottom.length);
      const paddedTopFrames =
        framesTop.length < maxFrameCount
          ? framesTop.concat(
              Array(maxFrameCount - framesTop.length).fill(
                framesTop[framesTop.length - 1],
              ),
            )
          : framesTop;
      const paddedBottomFrames =
        framesBottom.length < maxFrameCount
          ? framesBottom.concat(
              Array(maxFrameCount - framesBottom.length).fill(
                framesBottom[framesBottom.length - 1],
              ),
            )
          : framesBottom;
      ...
      for (let i = 0; i < paddedBottomFrames.length; i++) {
        ...
        if (!exists) {
          console.warn(
            `Bottom frame missing at index ${i}, waiting for file...`,
          );
          await this.delay(200);
          exists = await RNFS.exists(filePath);
          if (!exists) {
            console.warn(
              `Bottom frame still missing at index ${i}, writing duplicate of the last frame.`,
            );
            await this.writeFrameFile(
              filePath,
              paddedBottomFrames[paddedBottomFrames.length - 1],
            );
      ...
      FFmpegKit.executeAsync(
        ...
        async session => {
          ...
          if (returnCode.isValueSuccess()) {
            Alert.alert(
              'Recording Saved',
              `Recording saved to ${outputFilePath}`,
            );
          } else {
      ...
      if (frames.length === 0) {
        Alert.alert(
          'No frames captured',
          'Recording did not capture any frames.',
        );
        this.setState({isExporting: false});
        return;
      }
      const averageFPS =
        Math.round(frames.length / recordingDurationInSeconds) || 30;
      ...
      FFmpegKit.executeAsync(
        ...
        async session => {
          ...
          if (returnCode.isValueSuccess()) {
            Alert.alert(
              'Recording Saved',
              `The recording has been saved to ${outputFilePath}`,
            );
          } else {
            Alert.alert(
              'Recording Failed',
              'There was an error exporting the recording.',
            );
          }
          ...
          <View style={styles.splitContainer}>
            <View style={styles.splitSection}>
              ...
              {this.props.showFps && (
                <Text
                  ...
                  accessibilityLabel={`Frames per second top: ${fpsTop}`}
                  accessibilityRole="text">
                  Top FPS: {fpsTop}
                </Text>
              )}
            ...
            <View style={styles.splitSection}>
              ...
              {this.props.showFps && (
                <Text
                  style={styles.fpsText}
                  accessibilityLabel={`Frames per second bottom: ${fpsBottom}`}
                  accessibilityRole="text">
                  Bottom FPS: {fpsBottom}
                </Text>
              )}
            </View>
          </View>
